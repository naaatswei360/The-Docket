...
export const tocFinalExercise = {
  prompt:
    "Your document has a main heading, 'II. Statement of Facts', and a subsection under it, 'B. Procedural History'. Explain, in order, what you would do to get both onto an automatic Table of Contents with correct page numbers — including how you'd keep it accurate after a later edit.",
  modelAnswer:
...
export function gradeTocFinalExercise(answer) {
  const text = answer.trim();
  const checks = [
    {
      label: 'Applies Heading 1 to the main section',
      pass: /heading\s*1/i.test(text),
    },
    {
      label: 'Applies Heading 2 to the subsection',
      pass: /heading\s*2/i.test(text),
    },
    {
      label: "Inserts the table via References > Table of Contents",
      pass: /references/i.test(text) && /table of contents/i.test(text),
    },
    {
      label: "Mentions updating the field/table after edits",
      pass: /update/i.test(text),
    },
  ];
...
export const toaFinalExercise = {
  prompt:
    "You've just added a new citation to a footnote: Data Protection Act 2023, s 5. Explain, in order, what you would do to get it correctly listed in the Table of Authorities under the right category, with the table showing accurate page numbers.",
  modelAnswer:
...
export function gradeToaFinalExercise(answer) {
  const text = answer.trim();
  const checks = [
    {
      label: 'Mentions marking the citation (Mark Citation)',
      pass: /mark(ing)?\s+(the\s+)?citation/i.test(text),
    },
    {
      label: 'Assigns it to the correct category (Statutes/Legislation)',
      pass: /(statute|legislation)/i.test(text),
    },
    {
      label: "Uses References > Table of Authorities",
      pass: /references/i.test(text) && /table of authorities/i.test(text),
    },
    {
      label: 'Mentions updating the table after adding the citation',
      pass: /update/i.test(text),
    },
  ];
  const score = checks.filter((c) => c.pass).length;
